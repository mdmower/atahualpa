...
if ($comments) : ?>
	...
	<?php // Oldest or newest comments on top ?
	if ($bfa_ata['comment_display_order'] == "Oldest on top") { 
	...
	foreach ($all_comments as $comment) : 
		...
		OR $bfa_ata['separate_trackbacks'] == "No" ) { ?>
				...
				if ($bfa_ata['avatar_size'] != 0 AND $bfa_ata['avatar_size'] != "") {
					if (function_exists('get_avatar')) {
						...
						if(!empty($comment -> comment_author_email)) {
							...
							$default = urlencode($templateURI) . '/images/no-gravatar.gif');
							echo '<img class="avatar" src="http://www.gravatar.com/avatar.php?gravatar_id='.$md5.
							'&size='.$bfa_ata['avatar_size'].'&default='.$default.' alt="'. __('Gravatar','atahualpa') .'" />';
						}
					}
				}
			} ?>
			...
			<?php edit_comment_link(__('Edit','atahualpa'),'<span class="editcomment">','</span>'); ?>
		...
		foreach ($all_comments as $comment) : 
			if ( get_comment_type() != 'comment') { ?>
				...
				<li <?php echo $oddcomment; ?>id="comment-<?php comment_ID() ?>">
					...
					<div class="comment-number"><a href="#comment-<?php comment_ID() ?>" title="">
					<?php echo $comment_number; ?></a></div>
...
<?php else : // If there are NO comments ?>
	...
	<?php if ('open' == $post->comment_status) : // If comments are open, but there are no comments: ?>
		...
		<!--  <p><?php _e('There are no comments yet.','atahualpa'); ?></p> -->
	...
	<?php // If Login is not required, or User is logged in 
		...
		<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
			...
			<?php if ( $user_ID ) : // If User is logged in  ?>
				...
				<p><?php printf(__('Logged in as %s.', 'atahualpa'), '<a href="' . get_option('siteurl') . 
				'/wp-admin/profile.php">' . $user_identity . '</a>')?> 
				<a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="
				<?php _e('Log out of this account','atahualpa'); ?>"><?php _e('Logout &raquo;','atahualpa'); ?></a></p>

			<?php else :// If User is not logged in: Display the form fields "Name", "Email", "URL"  ?>
				...
				<p>
				<input class="text email" type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>
				" size="30" tabindex="2" />
		...
		<?php if (function_exists('lmbbox_comment_quicktags_display')) { 
			...
			if ($bfa_ata['show_xhtml_tags'] == "Yes") {	?>
				<p class="thesetags clearfix"><?php printf(__('You can use %1$sthese HTML tags</a>','atahualpa'), 
				'<a class="xhtmltags" href="#" onclick="return false;">'); ?></p>
				<div class="xhtml-tags"><p><code><?php echo allowed_tags(); ?></code></p></div><?php 
			}	
		} ?>
