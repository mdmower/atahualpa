...
if ( is_page() AND (function_exists('is_front_page') ? !is_front_page() : '') AND !is_home() ) {
	...
	if ($bfa_ata['left_col2_pages_exclude'] != "") { 
		...
		if ( $bfa_ata['leftcol2_on']['page'] AND !in_array($current_page_id, $pages_exlude_left2) ) {
			$cols++; $left_col2 = "on";
		}
	} else {
	...
	if ($bfa_ata['right_col2_pages_exclude'] != "") { 
		...
		if ( $bfa_ata['rightcol2_on']['page'] AND !in_array($current_page_id, $pages_exlude_right2) ) {
			$cols++; $right_col2 = "on"; 
		}
	} else {
