...
Description: Returns a list of the related entries based on active/passive keyword matches. Modified by wordpress.bytesforall.com to return instead of print result, to avoid the "No related posts" message. 
Version: 2.04
...
global $wpdb, $post;
	...
	// Get option values from the options page
  	...
  	if ($limit == "") { $limit = get_option('limit'); }
  	if ($len == "") { $len = get_option('len'); }
  	if ($before_title == "") { $before_title = stripslashes(get_option('before_title')); }
  	if ($after_title == "") { $after_title = stripslashes(get_option('after_title')); }
  	if ($before_post == "") { $before_post = stripslashes(get_option('before_post')); }
  	if ($after_post == "") { $after_post = stripslashes(get_option('after_post')); }
  	if ($show_pass_post == "") { $show_pass_post = get_option('show_pass_post'); }
	if ($show_excerpt == "") { $show_excerpt = get_option('show_excerpt'); }
